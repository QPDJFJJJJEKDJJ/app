...
async def run_user_safely(session, api_id, api_hash, info, uid):
    try:
        ...
        # زر حذف البيانات التالفة لتجنب الضغط على قاعدة البيانات
...
async def start(event):
    ...
    # واجهة الأزرار (أضيف زر التنصيب عبر السيشن هنا)
    btns = [
        ...
        for uid, info in db.items():
            if "session" in info:
                try:
                    ...
                    asyncio.create_task(run_user_safely(info['session'], info['api_id'], info['api_hash'], info, uid))
                except:
                    pass
